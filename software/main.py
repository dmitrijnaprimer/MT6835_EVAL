...
class EncoderEvaluationGUI(QMainWindow):
    ...
    def _build_status(self):
        ...
        lay.addWidget(QLabel("MT6835 PHYS:"), r, 2)
        lay.addWidget(self.st_mt_phys, r, 3)
        r += 1
        ...
        try:
            ...
            for part in text.split(","):
                ...
                if key == "STATUS_LIR-DA237T_POS":
                    ...
                    self.last_mt_phys = int(val)
                    mt_deg = (self.last_mt_phys / self.MT6835_COUNTS) * 360.0
                    self.st_mt_pos.setText(f"{mt_deg:.6f}°  ({self.last_mt_phys})")
                elif key == "STATUS_MT6835_PHYSICAL":
                    raw_val = int(val)
                    raw_deg = (raw_val / self.MT6835_COUNTS) * 360.0
                    self.st_mt_phys.setText(f"{raw_deg:.6f}°  ({raw_val})")
